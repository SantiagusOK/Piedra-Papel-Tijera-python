...
class Ppt:
    def __init__(self):
        ...
        self.imagen_nada_azul = PhotoImage(file = "fondos/nada_azul.png")
        self.imagen_nada_rojo = PhotoImage(file = "fondos/nada_rojo.png")
        self.eleccion_tu = Label(ventana,borderwidth=0,image=self.imagen_nada_azul)
        self.eleccion_op = Label(ventana,borderwidth=0,image=self.imagen_nada_rojo)
        self.etiqueta_op_name = Label(ventana,font=fuente_etiquetas_nombres,text=self.nombre_op,bg="black",fg="white",anchor="center",height=1,width=20)
        self.etiqueta_user_name = Label(ventana,font=fuente_etiquetas_nombres,text=self.nombre_tu,bg="black",fg="white",anchor="center",height=1,width=20)

    def dibujar_nombres(self):
        self.etiqueta_user_name.place_configure(x=50,y=67)
        self.etiqueta_op_name.place_configure(x=845,y=67)
        self.actualizar_nombres()
    
    def actualizar_nombres(self):
        self.etiqueta_user_name.configure(text=juego.nombre_tu)
        self.etiqueta_op_name.configure(text=juego.nombre_op)

    def quitar_nombres(self):
        self.etiqueta_user_name.place_configure(x=2000)
        self.etiqueta_op_name.place_configure(x=2000)
    
    def mostrar_elecciones(self):
        self.eleccion_tu.place(x=133,y=207)
        self.eleccion_op.place(x=756,y=207)
    
    def quitar_elecciones(self):
        self.eleccion_tu.place_configure(x=2000)
        self.eleccion_op.place_configure(x=2000)
...
class BotonesFinPartida:
    ...
    def boton_fin_partida(self):
        self.boton_JugarDeNuevo = tk.Button(image=self.imagen_bJugarDeNuevo,height=173,width=245,bg="white",relief="flat",borderwidth=0,command=self.Reiniciar_partida)
        ...
        self.boton_salirPartida.configure(command=self.quitar_FinPartida)
    ...
    def Reiniciar_partida(self):
        ...
        juego.quitar_nombres()
        puntos.quitar_resultados()
        mostrar_ronda()
    
    def quitar_FinPartida(self):
        juego.quitar_nombres()
        reproductor.sonar_seleccion1()
        puntos.reiniciar_resultados()
        puntos.quitar_resultados()
...
class BotonesMenu:
    ...
    def estadisticas(self):
        juego.dibujar_nombres()
        juego.etiqueta_user_name.place_configure(x=170,y=230)
        juego.etiqueta_op_name.place_configure(x=720,y=230)
...
class Puntuación:
    def __init__(self):
        self.fuente_marcador = ("Gill Sans MT", 50)
        self.puntos_tu = 0
        self.puntos_op = 0
        self.ganadas = 0
        self.perdidas = 0
        self.resultados = ""
        self.etiqueta_resultados = tk.Label(ventana,text=f"{self.puntos_tu}  -  {self.puntos_op}",bg="black",fg="white",font=self.fuente_marcador,height=1,width=15)
    ...
    def dibujar_resultados(self):
        self.etiqueta_resultados.place_configure(x=545,y=50)  
    
    def actualizar_resultados(self):
        self.etiqueta_resultados.configure(text=f"{self.puntos_tu}  -  {self.puntos_op}",bg="black",fg="white",font=self.fuente_marcador,height=1,width=5)
...
class ContadorRondas:
    def __init__(self):
        self.fuente_etiqueta_rondas = ("Fixedsys",100)
        self.ronda = 0
        self.etiqueta_ronda = Label(ventana,font=self.fuente_etiqueta_rondas,bg="black",fg="white")
        self.etiqueta_ronda.place(x=2000)
    
    def dibujar_rondas(self):
        self.etiqueta_ronda.place_configure(x=485,y=280)
...
class BotonesConfiguracion:
    ...
    def mostrar_boton_volumen_musica(self):
        self.boton_volumen = Button(ventana,image=self.imagen_bvolumen_on,height=84,width=144)
        self.boton_volumen.place(x=1058,y=145)
        self.revisar_mute_y_boton_musica()
    
    def mostrar_boton_volumen_sonido(self):
        self.boton_volumen_sonido = Button(ventana,image=self.imagen_bvolumen_on,height=84,width=144)
    ...
    def revisar_mute_y_boton_musica(self):
        if reproductor.mute_musica:
            self.boton_volumen.configure(image=self.imagen_bvolumen_off,command=self.desmutear_musica)
        else:
            self.boton_volumen.configure(image=self.imagen_bvolumen_on,command=self.mutear_musica)
    
    def revisar_mute_y_boton_sonido(self):
        if reproductor.mute_sonido:
            self.boton_volumen_sonido.configure(image=self.imagen_bvolumen_off,command=self.desmutear_sonido)
        else:
            self.boton_volumen_sonido.configure(image=self.imagen_bvolumen_on,command=self.mutear_sonido)
...
#--------------------------------------------------------------FUNCIONES--------------------------------#
def piedra():
    juego.mostrar_elecciones()
    sumar_estadisticas.piedra_tu += 1
    reproductor.sonar_seleccion2()
    botones_juego.quitar_botones_juego()
    juego.eleccion_tu.configure(image=imagen_piedra_tu)
    eleccion_bot = random.randint(1,3)

    if eleccion_bot == 1:#--------el bot eligio PIEDRA-----------------#
        sumar_estadisticas.piedra_op += 1
        juego.eleccion_op.configure(image=imagen_piedra_op)
        tiempo_esperar()

    if eleccion_bot == 2:#--------el bot eligio PAPEL-----------------#
        sumar_estadisticas.papel_op += 1
        juego.eleccion_op.configure(image=imagen_papel_op)
        puntos.punto_op()
        puntos.revisar_resultados()

    if eleccion_bot == 3:#--------el bot eligio TIJERA-----------------#
        sumar_estadisticas.tijera_op += 1
        juego.eleccion_op.configure(image=imagen_tijera_op)
        puntos.punto_tu()
        puntos.revisar_resultados()
...
def papel():
    ...
    reproductor.sonar_seleccion2()
    botones_juego.quitar_botones_juego()
    juego.eleccion_tu.configure(image=imagen_papel_tu)
    eleccion_bot = random.randint(1,3)
...
def tijera():
    ...
    reproductor.sonar_seleccion2()
    botones_juego.quitar_botones_juego()
    juego.eleccion_tu.configure(image=imagen_tijera_tu)
    eleccion_bot = random.randint(1,3)
...
def mostrar_pantalla_fin():
    ...
    juego.quitar_elecciones()
    puntos.etiqueta_resultados.place_configure(y=300)
    juego.etiqueta_op_name.place_configure(y=330)
    juego.etiqueta_user_name.place_configure(y=330)
    botones_fin.boton_fin_partida()
...
def contador_fin():
    t1 = Timer(7, quitarNombres_y_seguirRonda)
    t1.start()
...
def quitarNombres_y_seguirRonda():
    puntos.quitar_resultados()
    juego.quitar_elecciones()
    juego.quitar_nombres()
    mostrar_ronda()
